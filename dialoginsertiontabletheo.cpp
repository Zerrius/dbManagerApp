...
void DialogInsertionTableTheo::createNewLigne()
{
    ...
    primarykey=new QCheckBox(ui->tableWidgetInsertion);
    ...
    ui->tableWidgetInsertion->setCellWidget(ligne,3,primarykey);
}
/**
...
void DialogInsertionTableTheo::on_pushButtonEnregistrer_clicked()
{
    ...
    QString txtRequeteCreationTable = "CREATE TABLE "+takeNameTable()+"(";
    ...
    for (int nombreLigne = 0; nombreLigne < ui->tableWidgetInsertion->rowCount(); nombreLigne++)
    {
        ...
        bool key = ((QCheckBox*)(ui->tableWidgetInsertion->item(nombreLigne,3)))->isChecked();
        qDebug()<<"Bool"<<key;
        QString Prymarykey;
        if(key){Prymarykey="Primary Key";}
        else{Prymarykey="";}
        qDebug()<<"Key :"<<Prymarykey;
        if(nombreLigne!=0){
            txtRequeteCreationTable +=",";
        }
        qDebug()<<((QComboBox*)(ui->tableWidgetInsertion->cellWidget(nombreLigne,1)))->currentText();
        txtRequeteCreationTable +=nom+" "+((QComboBox*)(ui->tableWidgetInsertion->cellWidget(nombreLigne,1)))->currentText()+" "+((QLineEdit*)(ui->tableWidgetInsertion->cellWidget(nombreLigne,2)))->text()+" "+Prymarykey+"";
